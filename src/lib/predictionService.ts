export type MonthlyFinancialPoint = {
  ...
  function buildClientRiskSignals(
    ...
    const imbalance =
      revenueForecast > 0 ? Math.min(expensesForecast / revenueForecast, 1) : 1;
  
    const delayProbability =
      riskLevel === "Ridicat" ? 0.78 : riskLevel === "Mediu" ? 0.46 : 0.18;
  ...
  function buildChurnSignals(points: MonthlyFinancialPoint[]): ChurnSignal[] {
    ...
    const dropPercent =
      previousActivity > 0
        ? ((previousActivity - lastActivity) / previousActivity) * 100
        : 0;
  ...
  export function buildAiFinancialForecast(
    ...
    const orderedPoints = [...monthlyPoints].sort((a, b) =>
    ...
    const cashFlowScenario = buildCashFlowScenario(
      revenueForecast,
      expensesForecast,
      vatForecast,
    );
    ...
    const paymentDelayRisk = getPaymentDelayRisk(
      revenueForecast,
      expensesForecast,
      cashFlowForecast,
    );
